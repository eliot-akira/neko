...
function cE(i) {
  l ? (d.layers[i] = new Layer(0), eval("document." + i + "=d.layers[i]")) : void 0 !== d.createElement && (X = "<div id='" + i + '\' style="position:absolute">&nbsp;</div>', Y = d.createElement("DIV"), Y ? (Y.innerHTML = X, d.body.appendChild(Y)) : void 0 !== d.body.insertAdjacentHTML && d.body.insertAdjacentHTML("BeforeEnd", X));
}
function byName(e, t) {
...
function Neko(x, y, active, imagedirectory) {
  ...
  }, x || (x = 0), y || (y = 0), parseInt(x) != x ? (this.homeXfn = x, this.homeYfn = y, this.findHome(), x = this.homeX, y = this.homeY) : (x = parseInt(x), y = parseInt(y)), active || (active = false), !imagedirectory && window.NekoType && (imagedirectory = window.NekoType), imagedirectory || (imagedirectory = "i"), window.remoteimages || (imagedirectory = "https://webneko.net/" + imagedirectory), this.directory = imagedirectory;
...
function NekoCalculateDistance() {
  this.distx = this.endx - this.layer.myx, this.disty = this.endy - this.layer.myy, this.steps = Math.sqrt(Math.pow(this.distx, 2) + Math.pow(this.disty, 2)) / this.stepsize, this.steps >= 1 ? (this.caught && eval(this.onUnCaught), this.caught = false) : (this.caught || eval(this.onCaught), this.caught = true), this.dx = this.distx / this.steps, this.dy = this.disty / this.steps;
}
...
function NekoUpdateImage() {
  var paImages = eval("aNekos[" + this.whichNeko + "].a_" + this.behavior);
  if (this.frame >= paImages.length) if (this.behaviorRepetition++, 0 != this.loopTimes && this.behaviorRepetition >= this.loopTimes) {
...
function NekoMoveAStep() {
  this.steps >= 1 ? (this.layer.myx += this.dx, this.layer.myy += this.dy) : (this.layer.myx = this.endx, this.layer.myy = this.endy), box.checkBoard(this.whichNeko, this.layer.myx, this.layer.myy) && (this.layer.myx += this.eccX, this.layer.myy += this.eccY), sX(this.layer, this.layer.myx), sY(this.layer, this.layer.myy), box.setBoard(this.whichNeko, this.layer.myx, this.layer.myy);
}
function NekoFindDirection() {
  if (0 != t || 0 != e) {
    var e = -1 * this.dy, t = this.dx, i = Math.abs(t), o = "", s = "", a = Math.abs(e) / i, r = a < 0.41421, h = a > 2.4142;
...
function nekoChooseIdle() {
  var e = new Array("resting", "yawning", "itching", "scratching"), t = Math.floor(Math.random() * e.length);
  this.SetBehavior(e[t]);
}
function startANeko() {
  var e = 0, t = 0;
  window.startNekoX && (e = window.startNekoX), window.startNekoY && (t = window.startNekoY), parseInt(e) != e && (window.onresize = function () {
...
function boxSetBoard(e, t, i) {
  var o = Math.floor(t / this.width() * checkerboardScale), s = Math.floor(i / this.height() * checkerboardScale);
